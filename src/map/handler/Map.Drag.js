...
export const Drag = Handler.extend({
  ...
  _onPreDragLimit() {
    ...
    const limit = this._offsetLimit;
		...
		// ea-change: Improved pre-drag-limit
		// if (offset.x < limit.min.x) { offset.x = this._viscousLimit(offset.x, limit.min.x); }
		// if (offset.y < limit.min.y) { offset.y = this._viscousLimit(offset.y, limit.min.y); }
		// if (offset.x > limit.max.x) { offset.x = this._viscousLimit(offset.x, limit.max.x); }
		// if (offset.y > limit.max.y) { offset.y = this._viscousLimit(offset.y, limit.max.y); }

    const b = latLngBounds(this._map.options.maxBounds);
    const north = this._map.latLngToContainerPoint(b.getNorthWest()).y;
    const south = this._map.latLngToContainerPoint(b.getSouthEast()).y;
    const west = this._map.latLngToContainerPoint(b.getNorthWest()).x;
    const east = this._map.latLngToContainerPoint(b.getSouthEast()).x;
